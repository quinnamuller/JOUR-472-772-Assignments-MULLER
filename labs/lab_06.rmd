...
ACC_911 <- bind_rows(allegany_911, carroll_911, cecil_911)
```
...
balt_zcta <- baltimore_911 |> left_join(maryland_zcta, join_by(zip_code==zcta5n))
```
...
A2 For the first part, they all fall around the 30s, so I guess that Allegany having 17 calls in November is very unusual. Cecil and Allegany counties also breaking the 50-call mark is also an outlier. The location with the most calls is 314 GROVE NECK RD EARLEVILLE, MD, which is a rehab center.

```{r}
acc_months <- ACC_911 %>%
  mutate(month = month(date))

acc_months_totals <- acc_months %>%
