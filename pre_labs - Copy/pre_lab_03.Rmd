...
**Answer: 21202 which is Baltimore. This makes sense because, while it is not the capital of Maryland, it is where a lot of government offices are in the state, which is where money may be exchanged. It is also the center of Maryland's tourism efforts because of its history and attractions.**

```{r}
cleaned_md_grants_loans |>
  filter(str_detect(source, "MARYLAND TOURISM BOARD")) %>% 
  group_by(zip5) |>
  summarise(amtpaid_tozip5 = sum(amount)) |>
  arrange(desc(amtpaid_tozip5))
```
