...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer: They look just like the table from the website, but in the form of a dataframe. The dates are in name format as well.**
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **Answer: It may be helpful to see the span of each press release (ex: school-wide, district-wide, state-wide). It may also be helpful to see the specificity of that (ex: High Point High School, Prince George's County Public Schools, Maryland Department of Education)**
...
tags there are in the HTML source? **Answer: It seems like there are 96 <ul> tags..?**
...
tags. Describe the output: **Answer: For the most part, the output consists of the entries in the table, but it also includes all of the text from the navigation and social media tabs that I would normally assume are <div> tags or part of some Foundation code.**
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer: This created a table of all of the text entries from the previous output.**
