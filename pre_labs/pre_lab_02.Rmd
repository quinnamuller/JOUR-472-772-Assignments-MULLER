...
**Answer: We don't want to delete the entire dataset, the data may still be of use to us.**
...
#issue of expenditure dates being stored as character
#use lubridate to change dates to be recognized as dates by R
```
...
**Answer: It replaces the column so all the dates to have dashes instead of slashes and it goes year-month-day**
...
**Answer: They end in 01, to represent the entire month.**
...
**Answer: The code is creating a new column with the total number of votes for each county. It is then creating another new column in the same dataframe with the percentage of those total votes that Wes Moore won.**
...
**Answer: This just sorts the data in descending order by which counties have the most votes for Wes Moore. This is because we used the arrange(desc()) function.**
...
**Answer: It's more or less the same but it's interesting to see how more diverse vs more rural counties stand on whether they wanted Moore to be governor or not.**
...
**Answer: There is str_to_lower() which puts all the characters in lowercase. There is str_to_title() which puts the first character of each word in uppercase. Finally, there is str_to_sentence() which puts the first character of each sentence in uppercase**
...
**Answer: The first two lines assigns creates a new table and assigns it a name. Then, it creates a "state" category and decides that if there's a case where its detected that the "address" column has the word, for example, " Maryland " in it, it will assign the value "MD" to the state column in the new table. This repeats for several other states. The .default = NA thing at the end just means that if an entry is not satisfied by any of these str_detect functions, then the "state" value will be NA.**

**My biggest concern here is that in D.C., a lot of the streets have state names. Already, looking through the data, I saw two contributions from an address on Maine Ave and another on New York Ave. Also, this could create a problem with looking for data from the state of Washington because it factors in data from Washington, D.C. addresses.**
...
**Answer: Yes and no. Of course Maryland would have the most money going to places in Maryland. Second up for the identifiable states is Washington. This was my concern before. Most of those entries are probably for Washington, D.C. rather than the state of Washington.**
...
1.  How much money is Washington vs Washington, D.C. receiving from the state of Maryland.

2.  A lot of D.C.'s streets are named after states. As the third largest recipient of money from Maryland, How do we know that those entries are not being labelled by the state in their street name but rather by the actual state name?

3.  Is there a way we could find how many professors work for multiple departments at UMD?
