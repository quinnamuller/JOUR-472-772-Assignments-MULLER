...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Answer: 4 columns and 3822 rows**

```{r}
baltimore_overdose_calls <- read_csv("data/baltco_911_calls.csv")
```
...
So if we have a thing, like a county, or a month, or a day of the week, and then a count of that group, we can make a bar chart.
...
**Answer: The first line assigns a name, baltimore_overdose_calls_by_month, to a new dataframe based on the baltimore_overdose_calls dataframe. The second line adds a new column to the dataframe, the month of each call. The third and fourth lines group the data by month and summarize it by the total number of calls in the corresponding month. The fifth line arranges the data by which month had to most calls to which month has the least.**

```{r}
baltimore_overdose_calls_by_month <- baltimore_overdose_calls |>
  mutate(month = month(date, label=TRUE)) |> 
  group_by(month) |> 
  summarize(total_calls = n()) |> 
  arrange(desc(total_calls))

baltimore_overdose_calls_by_month
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: All I see is a grey box.**
...
**Answer: I see a bunch of columns, one for each month that are as high as the number of calls for that given month.**
...
**Answer: Each bar is in order from left to right in ascending order of the amount of calls for that given month.**
...
**Answer: Now all of the months are on the y-axis and the total calls are on the x-axis.**
...
**Answer: The background is no longer grey, it's white.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer: The background is this grey-sh blue color, and the background lines are thicker and not a grid.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: A chart of the number of calls for each date for a year. There is one entry for Feb. 6 in 2022 and 2023, which causes the dataframe to have 366 variables rather than 365.**

```{r}
baltimore_overdose_calls_by_date <- baltimore_overdose_calls |>
  group_by(date) |>
  summarise(
    total_calls=n()
  )

baltimore_overdose_calls_by_date 
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: A line chare of the number of overdose calls each day for a year.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: There are a lot more labels on x-axis, but th edata looks the same.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: The labels are still pretty hard to see but each week is labelled at a 45 degree angle.**
