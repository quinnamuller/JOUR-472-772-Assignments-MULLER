...
```{r}
alldata <- alldata |> mutate(change = ((Population2010-Population2000)/Population2000)*100) |> arrange(desc(change))
```
...
Is the difference between Howard and everyone else significant? **Answer: No**
...
**Answer: Do schools in areas with higher median household incomes have more high school graduates who enrolled in college? This may be difficult to interpret because college enrollment can be impacted by a lot more than just money. It can be impacted by family values, demographics, local industry, college or career readiness programs in high schools, etc. Looking at the two high schools in Charles County with the zip code 20603, we can see that even with the median household income being the same, North Point High School has significantly 16 more percentage points of students going to college.**

```{r}
md_zipcodes <- get_acs(geography="zcta", variables = "B19013_001", state='MD', year=2019)

md_high_schools_with_acs <- md_high_schools |>
  left_join(md_zipcodes, by=c('zip'='GEOID'))

md_high_schools_with_acs <- md_high_schools_with_acs |>
...
**Answer: The high schools with 60% or more students going to college has around 112000 median household income. The high schools with 40% or fewer students going to college has around 69000 median household income. This is a difference of 43000, which is pretty significant.**
...
#Do very small high schools have higher or lower college enrollment rates than very big high schools? Consider other factors: location, ruralness, more resources in big schools, etc.

```{r}
md_high_schools_with_acs |>
  filter(high_school_graduates <= 150) |> 
  summarize(avg_enrollment = mean(percent_of_high_school_graduates_enrolled), count =n())

md_high_schools_with_acs |>
  filter(high_school_graduates >= 450) |> 
  summarize(avg_enrollment = mean(percent_of_high_school_graduates_enrolled), count = n())
```

**Answer: In Maryland, smaller high schools on average have around 41% of graduates enrolled in college. Bigger high schools on average have around 55% of graduates enrolled in college. Bigger high schools have significantly more students enrolled in colelge after graduation.**
