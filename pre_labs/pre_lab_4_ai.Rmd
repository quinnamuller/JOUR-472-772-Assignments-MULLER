...
```{r}
library(axolotr)
create_credentials(GROQ_API_KEY = "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")
```
...
#Was wondering why this felt so familiar and its b/c Derek used this whole thing as an example for his What are you Working On talk about AI and Data Journalism last semester lol
...
```{r}
class_question <- axolotr::ask(
  prompt = "In our last lab we sifted through grants and loans paid by the state of Maryland to various entities. We sifted through the data to see how much money the state gave to stem cell research each year. Using R, suggest some ways to create graphics or charts to show the change in Maryland's investment in stem cell research over the years.",
  model = "llama-3.1-8b-instant"
)
...
**Your comment: After using the response to actually find the answer to me question, I'd say it works for the most part. You do, however, need to be somewhat familiar with R to understand that the "\n"s that are written throughout the response are just line breaks. You also need to know that most of the \s in the response don't shouldn't be there. The response itself seems very dense but is actually pretty straightforward if youare familiar with R.**

```{r}
#sort all the data to get the values for a chart
...
stem_cell_research <- stem_cell_research |>
...
```{r}
#trying the first groq suggestion (line chart)
library(ggplot2)
ggplot(stem_cell_research, aes(x = fiscal_year, y = stem_cell_amount)) +
geom_line(color = "hotpink") +
labs(
  title = "Maryland's Investment in Stem Cell Research Over the Years", 
  subtitle = "Line Chart",
  x = "Year",
  y = "Investment Amount"
)
...
```{r}
#trying the second suggestion from groq--bar chart
