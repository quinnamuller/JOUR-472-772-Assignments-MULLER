...
In the space below, write the name of the business [course?] from the first row of the "name" ["title"?] column. 
...
**Answer: 1) assigning the actions to the umd_courses dataset, 2) grouping the data by term 3) creating a summary of the data that includes 4) a count of the classes from each of the categories created in the second line**
...
**Answer: It arranged the count of courses for each semester in descending order. Now, the semesters that have historically had the most classes are listed at the top. **
...
**Answer: There is an additional column name in the parentheses after the "group_by" function. This makes it so that table shows how many classes each department had each term.**
...
**Answer: This summary makes it so that the data is grouped by term to show the total number of seats and classes for every term. It then sorts all of this data from the term with the most total seats to the least.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: The summary now also includes the mean and median number of seats for a class each semester. The top terms are almost entirely fall semesters.**
...
**Answer: The data is now sorted by department, so all of the  counts are the number of classes each department has had each term from Winter 2017 to Summer 2025. There were also two columns added for the minimum and maximum number of seats a class within the department had over this eight year period. The data is sorted in descending order by the total number of seats each department had over these eight years.**
...
1.  Which departments support the greatest amount of students? The least?

2.  Are there any professors that students are taking significantly more than others for the same courses? Or professors who are teaching more sections of the same classes?

3.  Can we use this data to track patterns in enrollment? Or enrollment in certain majors/fields/programs?
