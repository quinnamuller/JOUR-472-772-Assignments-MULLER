...
```{r}
maryland_pop_2020 <- read_csv('data/maryland_population_2020.csv') %>% clean_names()
```
...
maryland_pop_2020 |> left_join(county_voters_2020, join_by(county)) |> nrow()
```
...
```{r}
# without nrow
maryland_pop_2020 |> left_join(county_voters_2020, join_by(county))
```
...
```{r}
maryland_pop_with_voters <- maryland_pop_2020 |> left_join(county_voters_2020)
```
...
```{r}
maryland_pop_with_voters_and_demographics <- maryland_pop_with_voters |> left_join(maryland_demographics, join_by(county==NAME))
```
...
```{r}
maryland_pop_with_voters_and_demographics |>
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer: Howard County has the largest Asian 18+ population while it doesn't have a distinctly large Democratic population.**
